...
exports.capturePayment = async (req, res) => {
  try {
    ...
    if (!coursesId || coursesId.length === 0) {
      return ApiResponseFactory.badRequest(res, 'Please provide at least one course ID');
    }
    ...
    for (const courseId of coursesId) {
      ...
      const alreadyEnrolled = await courseRepo.isStudentEnrolled(courseId, userId);
      if (alreadyEnrolled) {
        return ApiResponseFactory.badRequest(res, 'Student is already enrolled in one of the selected courses');
      }
    ...
    return ApiResponseFactory.serverError(res, error, 'Could not initiate order');
  }
};
...
exports.verifyPayment = async (req, res) => {
  try {
    ...
    if (!razorpay_order_id || !razorpay_payment_id || !razorpay_signature || !coursesId || !userId) {
      return ApiResponseFactory.badRequest(res, 'Payment failed — missing data');
    }
    ...
    if (expectedSignature !== razorpay_signature) {
      return ApiResponseFactory.badRequest(res, 'Payment verification failed — invalid signature');
    }

    // Enroll student in each course
    await _enrollStudents(coursesId, userId);

    return ApiResponseFactory.success(res, { message: 'Payment verified and student enrolled' });
  } catch (error) {
    return ApiResponseFactory.serverError(res, error, 'Error while verifying payment');
  }
};
...
const _enrollStudents = async (courseIds, userId) => {
  for (const courseId of courseIds) {
    ...
    if (!enrolledCourse) throw new Error(`Course not found during enrollment: ${courseId}`);
    ...
    const enrolledStudent = await userRepo.enrollInCourse(userId, courseId, courseProgress._id);
...
exports.sendPaymentSuccessEmail = async (req, res) => {
  try {
    ...
    if (!orderId || !paymentId || !amount || !userId) {
      return ApiResponseFactory.badRequest(res, 'Please provide all required fields');
    }
    ...
    return ApiResponseFactory.success(res, { message: 'Payment success email sent' });
  } catch (error) {
    return ApiResponseFactory.serverError(res, error, 'Could not send payment success email');
  }
};
