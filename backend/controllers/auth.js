...
const _extractName = (email) =>
  email.split('@')[0].split('.').map((p) => p.replace(/\d+/g, '')).join(' ');
...
exports.sendOTP = async (req, res) => {
  try {
    ...
    return ApiResponseFactory.serverError(res, error, 'Error while generating OTP');
  }
};
...
exports.signup = async (req, res) => {
  try {
    ...
    if (password !== confirmPassword) {
      return ApiResponseFactory.badRequest(res, 'Password and confirm password do not match');
    }

    const alreadyExists = await userRepo.findByEmail(email);
    if (alreadyExists) {
      return ApiResponseFactory.badRequest(res, 'User already registered — please login');
    }
    ...
    if (!recentOtp) {
      return ApiResponseFactory.badRequest(res, 'OTP not found — please request a new one');
    }
    ...
    return ApiResponseFactory.success(res, { message: 'User registered successfully' });
  } catch (error) {
...
exports.login = async (req, res) => {
  try {
    ...
    if (!user) {
      return ApiResponseFactory.unauthorized(res, 'You are not registered with us');
    }
...
exports.changePassword = async (req, res) => {
  try {
    ...
    if (newPassword !== confirmNewPassword) {
      return ApiResponseFactory.badRequest(res, 'New password and confirm password do not match');
    }
    ...
    return ApiResponseFactory.success(res, { message: 'Password changed successfully' });
  } catch (error) {
    return ApiResponseFactory.serverError(res, error, 'Error while changing password');
  }
};
